...
var showChatUI = function () {
  ...
  var loginUserEle = document.getElementById('login_user').children[0]
  loginUserEle.innerHTML = curUserId
  loginUserEle.setAttribute('title', curUserId)
}
// 登录之前不显示web对话框
...
var handlePresence = function (e) {
  ...
  if (e.type === 'deleteGroupChat') {
    ...
    var targetTmp2 = $('#' + curUserId + '-' + groupFlagMark + e.from)
    var target = null
...
var handleRoster = function (rosterMsg) {
  for (var i = 0; i < rosterMsg.length; i++) {
    ...
    if (contact.subscription === 'remove') {
      isexist = contains(bothRoster, contact)
      if (isexist) {
...
var handleError = function (e) {
  ...
  if (curUserId == null) {
    ...
    var msg = e.msg
    if (e.type === window.EASEMOB_IM_CONNCTION_SERVER_CLOSE_ERROR) {
      if (msg === '' || msg === 'unknown') {
        alert('服务器断开连接,可能是因为在别处登录')
      } else {
        alert('服务器断开连接')
      }
    } else if (e.type === window.EASEMOB_IM_CONNCTION_SERVER_ERROR) {
